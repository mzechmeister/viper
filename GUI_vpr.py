...
def get_parameters():
    ...
    # parameters x axis
    l = LabelFrame(frm_par, text='test', bg=bg_frame, bd=2)
    l.grid(row=4, column=0, sticky="news", padx=10, pady=5, ipady=5, columnspan=1)
    ...
    for i, cn in enumerate(par_groups):
        yi, xi =  divmod(i, 6)
        l = LabelFrame(frm_par, text=cn, bg=bg_frame, bd=2)        
        l.grid(row=6+yi, column=xi, sticky="news", padx=10, pady=5, ipady=5, columnspan=1)
        frm_par.grid_columnconfigure(i, weight=1)
        lfr_par.append(l)
...
win.configure(background=ttk.Style().lookup('TFrame', 'background'), padx=3, pady=6)   # same bg-color for win and Frame

style = ttk.Style(win)
style.configure('lefttab.TNotebook', tabposition='nw')
style.configure('TNotebook.Tab', font=(font_type, font_size+1, 'bold'))
...
def new(event):
    ...
    frm_rv.config(width=win_width-26)
    frm_par.config(width=win_width-26)
    frm_res.config(width=win_width-26)

win.bind("<Configure>", new)
win.columnconfigure(0, weight=1)

# Frame of RV Tab
frm_rv = Frame(tab_rv, height=fr_high, width=win_width-26, bg=bg_frame, bd=2, relief='groove')
...
frm_rv1 = LabelFrame(frm_rv, text="Input", bg=bg_frame, bd=2)
...
lfr_oset1.grid(row=2, column=0, sticky="news", padx=(10,0), pady=5, ipady=5, columnspan=1)

lfr_oset2 = LabelFrame(frm_rv, text="oset rvo 2", bg=bg_frame, bd=2)
lfr_oset2.grid(row=2, column=1, sticky="news", padx=10, pady=5, ipady=5, columnspan=1)

lfr_cen = LabelFrame(frm_rv, text="center RV values", bg=bg_frame, bd=2)
lfr_cen.grid(row=3, column=0, sticky="news", padx=(10,0), pady=5, ipady=5, columnspan=1)

lfr_plot = LabelFrame(frm_rv, text="Plotting Options", bg=bg_frame, bd=2)
lfr_plot.grid(row=3, column=1, sticky="news", padx=10, pady=5, ipady=5, columnspan=1)

lfr_other = LabelFrame(frm_rv, text="Other", bg=bg_frame, bd=2)
lfr_other.grid(row=4, column=0, sticky="news", padx=(10,10), pady=5, ipady=5, columnspan=2)
...
frm_par = Frame(tab_par, height=fr_high, width=win_width-26, bg=bg_frame, bd=2, relief='groove')
frm_par.grid(row=0, column = 0, sticky="news", padx=10, pady=10)
frm_par.grid_propagate(False)

lpar = LabelFrame(frm_par, text='Input',  bg=bg_frame, bd=2)
...
frm_res = Frame(tab_res, height=fr_high, width=win_width-26, bg=bg_frame, bd=2, relief='groove')
...
b_save = Button(lfr_other, text='Save', command = lambda: call_vpr('-save'), background="#fdfdfd", width=15)
...
b_nrvo = ttk.Button(frm_rv, text='Plot no-rv', width = 15, command = lambda: call_vpr('-plot nrvo'))
b_nrvo.pack(**conf)

b_rvo = ttk.Button(frm_rv, text='Plot o-rv', width = 15, command = lambda: call_vpr('-plot rvo'))
b_rvo.pack(**conf)

b_rvbjd = ttk.Button(frm_rv, text='Plot BJD-RV', width = 15, command = call_vpr)
b_rvbjd.pack(**conf)

b_fpar = Button(lpar,text='Search data file', command = lambda: bt_par(e_parf), background="#fdfdfd", width=15)
b_fpar.grid(row=1, column=2, sticky="ne", padx=10, pady=0)

b_par = ttk.Button(frm_par, text='Plot par', width = 15, command = lambda: call_vpr('-plot par '))
...
b_oset1_a.grid(row=0, column=0, sticky="ne", padx=10, pady=(0,5), columnspan=6)
b_oset1_n = ttk.Button(lfr_oset1, text='select none', style = 'B2.TButton', command = lambda: set_oset(cb_orders1, 0))
b_oset1_n.grid(row=0, column=0, sticky="ne", padx=100, pady=(0,5), columnspan=6)

b_oset2_a = ttk.Button(lfr_oset2, text='select all', style = 'B2.TButton', command = lambda: set_oset(cb_orders2, 1))
b_oset2_a.grid(row=0, column=0, sticky="ne", padx=10, pady=(0,5), columnspan=6)
b_oset2_n = ttk.Button(lfr_oset2, text='select none', style = 'B2.TButton', command = lambda: set_oset(cb_orders2, 0))
b_oset2_n.grid(row=0, column=0, sticky="ne", padx=100, pady=(0,5), columnspan=6)


b_fres = Button(lres,text='Search directory', command = lambda: bt_res(e_dir), background="#fdfdfd", width=15)
b_fres.grid(row=1, column=2, sticky="ne", padx=10, pady=0)

b_res = ttk.Button(frm_res, text='Plot res', width = 15, command = lambda: call_vpr(res=True))
#b_res.grid(row=12, column=1, sticky="se", padx=xy0, pady=(xy0,xy0))
b_res.pack(**conf)

b_reso_a = ttk.Button(lreso, text='select all', style = 'B2.TButton', command = lambda: set_oset(cb_reso, 1))
b_reso_a.grid(row=0, column=0, sticky="ne", padx=10, pady=(0,5), columnspan=6)
b_reso_n = ttk.Button(lreso, text='select none', style = 'B2.TButton', command = lambda: set_oset(cb_reso, 0))
b_reso_n.grid(row=0, column=0, sticky="ne", padx=100, pady=(0,5), columnspan=6)

b_resn_a = ttk.Button(lresn, text='select all', style = 'B2.TButton', command = lambda: set_oset(cb_resn, 1))
b_resn_a.grid(row=0, column=0, sticky="ne", padx=10, pady=(0,5), columnspan=6)
b_resn_n = ttk.Button(lresn, text='select none', style = 'B2.TButton', command = lambda: set_oset(cb_resn, 0))
b_resn_n.grid(row=0, column=0, sticky="ne", padx=100, pady=(0,5), columnspan=6)
...
e_run.pack(side='left', padx=(xy0,60), pady=(0, xy0))
...
Label(frm_rv, text='Plot RVs', font=(font_type, font_size, 'bold'), background=bg_frame).grid(row=0, column=0, sticky="nw", padx=xy0, pady=(xy0,5), columnspan = 2)

Label(frm_rv1, text='rvo file 1:', font=(font_type, font_size-1, 'bold'), background=bg_frame).grid(row=0, column=0, sticky="sw", padx=xy0, pady=5)

Label(frm_rv1, text='rvo file 2:', font=(font_type, font_size-1, 'bold'), background=bg_frame).grid(row=1, column=0, sticky="nw", padx=xy0, pady=5)
...
Label(lpar, text='parfile:', background=bg_frame, font=(font_type, font_size-1, 'bold')).grid(row=1, column=0, sticky="nw", padx=xy0, pady=5)

Label(frm_par, text='Parameters x-axis', font=(font_type, font_size-1, 'bold'), background=bg_frame).grid(row=3, column=0, sticky="nw", padx=xy0, pady=xy0, columnspan = 5)

Label(frm_par, text='Parameters y-axis', font=(font_type, font_size-1, 'bold'), background=bg_frame).grid(row=5, column=0, sticky="nw", padx=xy0, pady=xy0, columnspan = 5)
...
Label(lres, text='directory:', background=bg_frame, font=(font_type, font_size-1, 'bold')).grid(row=1, column=0, sticky="nw", padx=xy0, pady=3)
